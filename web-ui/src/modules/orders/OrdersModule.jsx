      ...
      {loading ? <div className="empty-state">Loading orders...</div> : (
        <>
          ...
          <section className="record-main-layout">
            ...
            <Panel title="Provisioning Jobs" description="Jobs returned by /api/ops/provisioning-jobs.">{jobs.length ? <DataTable columns={[{ key: "JobNumber", label: "Job" }, { key: "JobType", label: "Type" }, { key: "OwnerName", label: "Owner" }, { key: "Status", label: "Status", render: row => <StatusTag tone={statusTone(row.Status)}>{row.Status}</StatusTag> }, { key: "DueDate", label: "Due", render: row => formatDate(row.DueDate) }]} rows={jobs} /> : <div className="empty-state">No provisioning jobs returned by the ops API.</div>}</Panel>
          </section>
        </>
      )}
    </section>
