...
import { DataTable, MetricCard, Panel, StatusTag, formatDate, formatMoney } from "../../components/Primitives";
    ...
    <>
      ...
      {loading ? <div className="empty-state">Loading service operations...</div> : (
        <>
          ...
          {(route === "network" || route === "service-management") && <Panel title="Network Events" description="Network events returned by /api/ops/network-events." action={<button className="ghost-button" disabled={saving} type="button" onClick={() => createSample("network")}>Create sample event</button>}>{data.networkEvents.length ? <DataTable columns={[{ key: "EventNumber", label: "Event" }, { key: "Market", label: "Market" }, { key: "Type", label: "Type" }, { key: "Impacted", label: "Impacted" }, { key: "Severity", label: "Severity", render: row => <StatusTag tone={row.Severity === "Major" ? "warn" : "blue"}>{row.Severity}</StatusTag> }, { key: "Status", label: "Status" }, { key: "SlaExposure", label: "SLA Exposure", render: row => formatMoney(row.SlaExposure || 0) }]} rows={data.networkEvents} /> : <div className="empty-state">No network events returned by the ops API.</div>}</Panel>}
          {(route === "provisioning" || route === "service-management") && <Panel title="Provisioning Jobs" description="Provisioning jobs returned by /api/ops/provisioning-jobs." action={<button className="ghost-button" disabled={saving} type="button" onClick={() => createSample("provisioning")}>Create sample job</button>}>{data.provisioningJobs.length ? <DataTable columns={[{ key: "JobNumber", label: "Job" }, { key: "JobType", label: "Type" }, { key: "OwnerName", label: "Owner" }, { key: "Status", label: "Status", render: row => <StatusTag tone={["Completed", "Validated"].includes(row.Status) ? "success" : ["Queued", "Blocked", "In Progress"].includes(row.Status) ? "warn" : "blue"}>{row.Status}</StatusTag> }, { key: "DueDate", label: "Due", render: row => formatDate(row.DueDate) }]} rows={data.provisioningJobs} /> : <div className="empty-state">No provisioning jobs returned by the ops API.</div>}</Panel>}
          {route === "carrier-settlement" && <Panel title="Carrier Settlement" description="Carrier settlement records returned by /api/ops/carrier-settlement." action={<button className="ghost-button" disabled={saving} type="button" onClick={() => createSample("settlement")}>Create sample settlement</button>}>{data.settlements.length ? <DataTable columns={[{ key: "SettlementNumber", label: "Settlement" }, { key: "PartnerName", label: "Partner" }, { key: "BillingPeriod", label: "Period" }, { key: "ExposureAmount", label: "Exposure", render: row => formatMoney(row.ExposureAmount || 0) }, { key: "Status", label: "Status" }, { key: "ClaimType", label: "Claim" }]} rows={data.settlements} /> : <div className="empty-state">No carrier settlement rows returned by the ops API.</div>}</Panel>}
        </>
      )}
    </>
  );
