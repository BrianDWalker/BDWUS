...
export default function ServiceOpsModule({ route = "network", showToast }) {
  ...
  return (
    <>
      <PageHeader title={title} description="API-backed operations, network events, provisioning jobs, and carrier settlement." actions={<button className="button" type="button" onClick={loadOps}>Refresh</button>} />
      ...
      {loading ? <div className="empty-state">Loading service operations...</div> : (
        <>
          ...
          {(route === "network" || route === "service-management") && <Panel title="Network Events" description="Network events returned by /api/ops/network-events." action={<button className="ghost-button" disabled={saving} type="button" onClick={() => createSample("network")}>Create sample event</button>}><DataTable columns={[{ key: "EventNumber", label: "Event" }, { key: "Market", label: "Market" }, { key: "Type", label: "Type" }, { key: "Impacted", label: "Impacted" }, { key: "Severity", label: "Severity", render: row => <StatusTag tone={row.Severity === "Major" ? "warn" : "blue"}>{row.Severity}</StatusTag> }, { key: "Status", label: "Status" }, { key: "SlaExposure", label: "SLA Exposure", render: row => formatMoney(row.SlaExposure || 0) }]} rows={data.networkEvents} /></Panel>}
          {(route === "provisioning" || route === "service-management") && <Panel title="Provisioning Jobs" description="Provisioning jobs returned by /api/ops/provisioning-jobs." action={<button className="ghost-button" disabled={saving} type="button" onClick={() => createSample("provisioning")}>Create sample job</button>}><DataTable columns={[{ key: "JobNumber", label: "Job" }, { key: "JobType", label: "Type" }, { key: "OwnerName", label: "Owner" }, { key: "Status", label: "Status" }, { key: "DueDate", label: "Due" }]} rows={data.provisioningJobs} /></Panel>}
          {route === "carrier-settlement" && <Panel title="Carrier Settlement" description="Carrier settlement records returned by /api/ops/carrier-settlement." action={<button className="ghost-button" disabled={saving} type="button" onClick={() => createSample("settlement")}>Create sample settlement</button>}><DataTable columns={[{ key: "SettlementNumber", label: "Settlement" }, { key: "PartnerName", label: "Partner" }, { key: "BillingPeriod", label: "Period" }, { key: "ExposureAmount", label: "Exposure", render: row => formatMoney(row.ExposureAmount || 0) }, { key: "Status", label: "Status" }, { key: "ClaimType", label: "Claim" }]} rows={data.settlements} /></Panel>}
        </>
      )}
    </>
  );
