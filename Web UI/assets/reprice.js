...
function setReadonlyOpportunityFields(data) {
  ...
  if (!data) return;
  if (els.customerType) els.customerType.textContent = renderMetadataValue(data.customerType);
  if (els.industryType) els.industryType.textContent = renderMetadataValue(data.industryType);
  if (els.customerRegion) els.customerRegion.textContent = renderMetadataValue(data.customerRegion);
  if (els.countryCode) els.countryCode.textContent = renderMetadataValue(data.countryCode);
  if (els.customerStatus) els.customerStatus.textContent = renderStatusValue(data.customerStatus);
  if (els.creditRating) els.creditRating.textContent = renderMetadataValue(data.creditRating);
  if (els.planTier) els.planTier.textContent = renderMetadataValue(data.planTier);
  if (els.planName) els.planName.textContent = renderMetadataValue(data.planName);
  if (els.serviceName) els.serviceName.textContent = renderMetadataValue(data.serviceName);
  if (els.serviceCategory) els.serviceCategory.textContent = renderMetadataValue(data.serviceCategory);
  if (els.subscriptionQuantity) els.subscriptionQuantity.textContent = renderMetadataValue(data.subscriptionQuantity);
...
async function loadOpportunityDetails() {
  ...
  if (details && els.opportunityId) {
    els.opportunityId.textContent = opportunityId;
    if (els.currentQuoteId) {
      els.currentQuoteId.textContent = details.quoteId || details.opportunity?.quoteId || details.currentQuoteId || "--";
    }
...
function init() {
  const opportunityId = getOpportunityId();
  if (els.opportunityId) els.opportunityId.textContent = opportunityId || "--";
